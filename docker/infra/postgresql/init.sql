...
-- Add unique key constraints // 7.10 수정
create unique index uk_affiliation_corporation_id_department
    on affiliation (corporation_id, department)
...
-- Add unique key constraints 7.10
create unique index uk_affiliation_id_location
    on asset_location (affiliation_id, location)
    where deleted_at is null;
...
create unique index uk_id_asset_type_id
    on asset_type (id, asset_type_id)
    where deleted_at is null;
